...
class WC_Collection_Date_Calendar {
	...
	public function enqueue_calendar_assets( $hook ) {
		// Only load on our settings page when calendar tab is active.
		...
		wp_localize_script(
			...
			array(
				...
				'dayNamesMin' => array_values( $this->get_day_names_min() ),
				'texts'       => array(
	...
	public function ajax_get_calendar_data() {
		...
		$statistics    = $this->get_month_statistics( $year, $month );
	...
	public function ajax_update_capacity() {
		...
		$updated = $this->update_daily_capacity( $date, $capacity );
	...
	public function ajax_get_day_bookings() {
		...
		$bookings = $this->get_day_bookings( $date );
	...
	private function get_calendar_data( $year, $month ) {
		...
		$capacity_table = $wpdb->prefix . 'wc_collection_capacities';
		$capacities = $wpdb->get_results(
			$wpdb->prepare(
				"SELECT date, capacity FROM {$capacity_table} WHERE YEAR(date) = %d AND MONTH(date) = %d",
				$year,
				$month
			)
		);
	 ...
	 * @since 1.2.0
	 * @param string $date     Date in Y-m-d format.
	 * @param int    $capacity New capacity value.
	 * @return bool True if updated successfully.
	 */
	private function update_daily_capacity( $date, $capacity ) {
		global $wpdb;

		$table_name = $wpdb->prefix . 'wc_collection_capacities';
	 ...
	 */
	private function get_day_names_min() {
		return array(
